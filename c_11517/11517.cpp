...
//retorna un par con el menor precio a pagar y la cantidad de monedas usadas
//ptp es el precio a pagar, y c es la moneda actual (voy disminuyendo hasta llegar a 0)
pp minp (int ptp, int c){
  ...
  if (mem[ptp][c] == p_i){
    ...
    pp sec = minp(p, c-1);
    //si no la uso, busco en la moneda siguiente con el mismo ptp
